...
def goal_progress(goal, habits_with_logs: list, tasks: list, start: datetime, end: datetime) -> dict:
    habit_results = []
    for habit, logs in habits_with_logs:
        count = completions_in_range(habit.id, logs, start, end)
        rate = min(count / habit.frequency_target, 1.0) if habit.frequency_target else 0.0

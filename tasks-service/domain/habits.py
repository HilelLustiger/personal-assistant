...
def count_completions_in_range(habit_id, logs: list, start: datetime, end: datetime) -> int:
    return sum(
        1 for log in logs
...
def is_habit_hit_in_range(habit, logs: list, start: datetime, end: datetime) -> bool:
    return count_completions_in_range(habit.id, logs, start, end) < habit.frequency_target

...
def test_is_due_true_when_past_and_not_fired():
    ...
    r = _make_reminder(trigger_datetime=datetime(2026, 6, 23, 11, 0, tzinfo=timezone.utc))
...
def test_is_due_false_when_future():
    from domain.reminders import is_due
    r = _make_reminder(trigger_datetime=datetime(2026, 6, 24, 9, 0, tzinfo=timezone.utc))
    assert is_due(r, NOW) is False
